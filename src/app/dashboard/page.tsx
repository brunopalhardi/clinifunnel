  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        {/* Revenue by day */}
        <div className="rounded-xl bg-card p-6 glass-border">
          <h2 className="font-display text-lg font-semibold mb-5">Receita por periodo</h2>
          <div className="flex items-end gap-2 h-48">
            {d.revenueChart.map((r) => (
              <div key={r.day} className="flex flex-1 flex-col items-center gap-1">
                <span className="text-[10px] text-muted-foreground">{r.value > 0 ? fmtK(r.value) : ""}</span>
                <div className="w-full rounded-t-md bg-success/80 transition-all duration-500" style={{ height: `${maxRevenue > 0 ? (r.value / maxRevenue) * 140 : 4}px`, minHeight: r.value > 0 ? "8px" : "2px" }} />
                <span className="text-[11px] text-muted-foreground">{r.day}</span>
              </div>
            ))}
          </div>
        </div>
