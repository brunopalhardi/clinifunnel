...
- **[DASH-14] Aba Painel Principal (KPIs executivos da call do Sergio)** — PR #_TBD_ — v0.55.0
  Nova aba (virou a home do dashboard) com os 4 KPIs que o Sergio ditou na call de 08/06: ticket medio por PACIENTE global, paciente novo e paciente recorrente (cada card com qtd de pacientes + receita pra auditoria), e tabela por doutora (pacientes / receita / ticket medio). API /api/painel reusa a lib de [DASH-13]; por-doutora deriva de procedures (dentista 100% ali, 0% nos appointments — vistoria DASH-12). Sem fetch de dentista via appointments. Relacionado: [DASH-13].
